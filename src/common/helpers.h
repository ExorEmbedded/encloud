...
#define __ENCLOUD_PRINT(lev, levstr, msg) fprintf(stderr, "[%s] %s:%s:%s:%d] %s\n", ENCLOUD_PKGNAME, levstr, __FILE__, __FUNCTION__, __LINE__, qPrintable(msg))
#define ENCLOUD_PRINTF(...) fprintf(__VA_ARGS__)
...
#define ENCLOUD_EMIT_ERR(sig) do { ENCLOUD_ERR(""); emit(sig); goto err; } while (0)
#define ENCLOUD_EMIT_RETURN_IF(cond, sig, rc) do { if (cond) { ENCLOUD_ERR(""); emit(sig); return rc; } } while (0)
#define ENCLOUD_EMIT_RETURN_MSG_IF(cond, sig, msg, rc) do { if (cond) { ENCLOUD_ERR(msg); emit(sig); return rc; } } while (0)
#define ENCLOUD_EMIT_ERR_IF(cond, sig) do { if (cond) { ENCLOUD_ERR(""); emit(sig); goto err; } } while (0)
#define ENCLOUD_EMIT_ERR_MSG_IF(cond, sig, msg) do { if (cond) { ENCLOUD_ERR(msg); emit(sig); goto err; } } while (0)
